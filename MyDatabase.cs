...
namespace NPoco
{
    public partial class MyDatabase : IDatabase
    {
        ...
        static MyDatabase()
        {
            regexList = new List<Regex> {
                    new Regex(@"insert[ ]+into[ ]+[\[|`|""]*([_0-9a-zA-Z]+)[\]|`|""]*", RegexOptions.IgnoreCase),
                    new Regex(@"delete[ ]+from[ ]+[\[|`|""]*([_0-9a-zA-Z]+)[\]|`|""]*", RegexOptions.IgnoreCase),
                    new Regex(@"update[ ]+[\[|`|""]*([_0-9a-zA-Z]+)[\]|`|""]*", RegexOptions.IgnoreCase),
                    new Regex(@"select[ ]+.+[ ]+from[ ]+[\[|`|""]*([_0-9a-zA-Z]+)[\]|`|""]*", RegexOptions.IgnoreCase)
                };
        }

        private void ReInit(string connectionString, string providerName, IsolationLevel? isolationLevel, bool enableAutoSelect)
        {
        ...
        public MyDatabase(DbConnection connection, DatabaseType dbType)
            : this(connection, dbType, null, DefaultEnableAutoSelect)
        { }
        
        public MyDatabase(DbConnection connection, DatabaseType dbType, IsolationLevel? isolationLevel)
            : this(connection, dbType, isolationLevel, DefaultEnableAutoSelect)
        { }

        public MyDatabase(DbConnection connection, DatabaseType dbType, IsolationLevel? isolationLevel, bool enableAutoSelect)
        {
            EnableAutoSelect = enableAutoSelect;
...
#if !DNXCORE50
        public MyDatabase(string connectionString, string providerName)
            : this(connectionString, providerName, DefaultEnableAutoSelect)
        { }

        public MyDatabase(string connectionString, string providerName, IsolationLevel isolationLevel)
            : this(connectionString, providerName, isolationLevel, DefaultEnableAutoSelect)
        { }

        public MyDatabase(string connectionString, string providerName, bool enableAutoSelect)
            : this(connectionString, providerName, null, enableAutoSelect)
        { }

        public MyDatabase(string connectionString, string providerName, IsolationLevel? isolationLevel, bool enableAutoSelect)
        {
            EnableAutoSelect = enableAutoSelect;
            KeepConnectionAlive = false;
        ...
        public MyDatabase(string connectionString, DatabaseType dbType)
            : this(connectionString, dbType, null, DefaultEnableAutoSelect)
        { }

        public MyDatabase(string connectionString, DatabaseType dbType, IsolationLevel? isolationLevel)
            : this(connectionString, dbType, isolationLevel,  DefaultEnableAutoSelect)
        { }

        public MyDatabase(string connectionString, DatabaseType dbType, IsolationLevel? isolationLevel, bool enableAutoSelect)
        {
            EnableAutoSelect = enableAutoSelect;
            KeepConnectionAlive = false;
        ...
        public MyDatabase(string connectionString, DatabaseType databaseType, DbProviderFactory provider)
            : this(connectionString, databaseType, provider, null, DefaultEnableAutoSelect)
        { }

        public MyDatabase(string connectionString, DatabaseType databaseType, DbProviderFactory provider, IsolationLevel? isolationLevel = null, bool enableAutoSelect = DefaultEnableAutoSelect)
        {
            EnableAutoSelect = enableAutoSelect;
            KeepConnectionAlive = false;
        ...
        public MyDatabase(string connectionStringName, IsolationLevel isolationLevel)
            : this(connectionStringName, isolationLevel, DefaultEnableAutoSelect)
        { }

        public MyDatabase(string connectionStringName, bool enableAutoSelect)
            : this(connectionStringName, (IsolationLevel?) null, enableAutoSelect)
        { }

        public MyDatabase(string connectionStringName, IsolationLevel? isolationLevel,  bool enableAutoSelect)
        {
            EnableAutoSelect = enableAutoSelect;
            KeepConnectionAlive = false;
        ...
        {
            if (string.IsNullOrEmpty(this.curTableName))
            {
                this.curTableName = "default";
                foreach (Regex regex in regexList)
                {
                    Match m = regex.Match(curSQL);
                    if (m.Success)
                    {
                        this.curTableName = m.Groups[1].Value;
                        break;
                    }
                }
            }
            Db db = DbConfig.GetDb(this.curTableName, this.curOpName);
            this.ReInit(db.ConnectionString, db.ProviderName, null, DefaultEnableAutoSelect);
        ...
        private string curTableName = "default";
        private string curOpName = "Write";
